package roadfriend.app.utils.helper.billing
...
import roadfriend.app.utils.helper.billing.BillingManager.Companion.formatPeriod
...
class BillingManager(
    ...
    fun queryBusinessSubscriptionSkuDetails(
        onSuccess: (List<SkuDetails>) -> Unit,
        onError: (code: Int, message: String) -> Unit
    ) {
        startServiceConnection {
            querySubscriptionSkuDetails(
                listOf(Sku.BUSINESS_MONTHLY, Sku.BUSINESS_YEARLY),
                onSuccess,
                onError
            )
        }
    ...
    fun queryIndividualSubscriptionSkuDetails(
        onSuccess: (List<SkuDetails>) -> Unit,
        onError: (code: Int, message: String) -> Unit
    ) {
        startServiceConnection {
    ...
    object Sku {
        const val BUSINESS_MONTHLY =
            "business_monthly2" // Made a mistake with "business_monthly", accidentally make it yearly, so we can't use that SKU anymore
...
fun SkuDetails.displayPeriod() =
    formatPeriod(this.subscriptionPeriod, isIncludeSingularNumber = false)

fun SkuDetails.displayIntroductoryPeriod() =
    formatPeriod(this.introductoryPricePeriod, isIncludeSingularNumber = true)